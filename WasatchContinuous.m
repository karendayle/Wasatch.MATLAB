...
% display summary
%fprintf('Found %s %s with %d pixels (%.2f nm)\n', ...
%    modelName, serialNum, pixels, wavelengths(1));
% display summary
...
if ((myAns1 == 1) || (myAns1 == 4))
    ...
    if (myAns1 == 0)
        fprintf('Skipping dark, using all zeros');
    else
        if (myAns1 == 2)
            fileID = -1;
            errmsg = '';
            while fileID < 0 
                disp(errmsg);
                filename = input ...
                    ('Open file in C:\\Users\dayle.kotturi\\Documents\\Data\\Official Dark: ', ...
                    's');
...
function a = takeSpectrum(numPoints, spectrometer, integrationTimeMS)
    ...
    pixels = spectrometer.pixels;
    if (pixels > numPoints)
        fprintf('Number of pixels exceeds space in arrays\n');
    end
    a = spectrum;
end
